...
invmatvecprod(L::LinearOperator{T,ND,NR}, u::AbstractArray{T,NR}) where {T,ND,NR} = ~inothing(L.invmatvecprod) && L.invmatvecprod(u)
invmatvecprod_adj(L::LinearOperator{T,ND,NR}, v::AbstractArray{T,ND}) where {T,ND,NR} = ~inothing(L.invmatvecprod_adj) && L.invmatvecprod_adj(v)
